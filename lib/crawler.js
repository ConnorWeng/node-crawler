...
Crawler.crawl = function(url, options) {
    var defer = Q.defer();
    var c = Crawler.instancePool.get(options);
    c.options.callback = function(errors, response) {
        if (errors) {
            defer.reject(errors);
        } else {
...
Crawler.instancePool = {
    pool: [],
    get: function(options) {
        for (var i = 0; i < this.pool.length; i++) {
            if (_.isEqual(this.pool[i].options, options)) {
                return this.pool[i].crawler;
            }
        }
        var crawler = new Crawler(options);
        this.pool.push({options: options, crawler: crawler});
        return crawler;
    },
    size: function() {
