...
exports.Crawler = function(options) {
    ...
    self.onContent = function (error, toQueue, response, fromCache) {
        ...
        if (toQueue.forceUTF8) {
            ...
            if (!toQueue.incomingEncoding) {
                ...
                if (detected && detected.encoding) {
                    ...
                    if (detected.encoding === "windows-1252") {
                    ...
                    if (detected.encoding != "utf-8" && detected.encoding != "ascii") {
                        ...
                        if (iconv) {
                            ...
                            // iconv-lite doesn't support Big5, EUC-TW (yet)
                        } else if (detected.encoding != "Big5" && detected.encoding != "EUC-TW") {
                ...
                if (iconv) {
                    ...
                    response.body = iconvObj.convert(response.body).toString();
                    // iconv-lite doesn't support Big5, EUC-TW (yet)
                } else if (toQueue.incomingEncoding != "Big5" && toQueue.incomingEncoding != "EUC-TW") {
        ...
        // FIXME: why response body does not have match function?
        if (!response.body.match) {
            toQueue.callback(new Error('response.body.match is undefined'), toQueue);
            return release(toQueue);
        }
